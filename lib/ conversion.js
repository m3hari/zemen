...
function setEra(era) {
    if ((JD_EPOCH_OFFSET_AMETE_ALEM === era) || (JD_EPOCH_OFFSET_AMETE_MIHRET === era)) {
        ...
        throw (new java.lang.ArithmeticException("Unknown era:"));
    }
}

function isEraSet() {
    return (JDN_OFFSET === JD_EPOCH_OFFSET_UNSET) ? false : true;
}
...
function ethiopicToGregorian(day, month, year) {
    if (!isEraSet()) {
        if (year <= 0) {
